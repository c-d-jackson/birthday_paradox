...
# Returns counts of birthday frequencies
sim_people <- function(n_people, n_birthdays) {
  birthdays <- sample(1:365, n_people, replace = TRUE)
  foo <- as.data.frame(table(birthdays))
  
  foo %>%
    summarize(n_people = n_people,
              count = sum(Freq >= n_birthdays)) %>%
   setNames(c("n_people", paste0("bd", n_birthdays)))
}

# Function to run multiple simulations for a given room size
run_simulations <- function( n_sims = 1000, n_people, n_birthdays) {
  results <- replicate(n_sims, sim_people(n_people, n_birthdays), simplify = F)
  bind_rows(results)
}

# tmp <- run_simulations(n_sims = 100, n_people = 25, n_birthdays = 2)
```
